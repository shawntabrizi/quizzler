...
describe("known contract deployments", () => {
    it("uses the active pair first and resolves only allowlisted historical pairs", () => {
        ...
        expect(catalog.map((deployment) => deployment.maxPlayers)).toEqual([16, 16]);
    ...
    it("gives legacy address files a deterministic deployment ID", () => {
        ...
        expect(deploymentCatalog({ registry: CURRENT.registry, game: CURRENT.game })[0]?.maxPlayers)
            .toBe(LEGACY_MAX_LOBBY_PLAYERS);
    });
});
